...
export function AuthModal({ isOpen, onClose, onSuccess, language, demoTitle }: AuthModalProps) {
  ...
  const handleBiometricRegister = async () => {
    ...
    setAuthError('');
    try {
      // Require password before allowing biometric register
      const entered = typeof window !== 'undefined' ? window.prompt(language === 'vi' ? 'Nhập mật khẩu để đăng ký sinh trắc học' : 'Enter password to register biometrics') : '';
      if ((entered || '').trim() !== 'orihara') {
        throw new Error(language === 'vi' ? 'Mật khẩu không đúng' : 'Invalid password');
      }
      ...
      setAuthError(
        (language === 'vi' ? 'Đăng ký sinh trắc học thất bại: ' : 'Biometric registration failed: ') + message
      );
      return;
    } finally {
      setIsAuthenticating(false);
